...
class test {
    __New(){
        ...
        hk := new _CHotkeyControl(this, "MyHotkey", callback, "x5 y5 w100", "F12")
        Gui, Show, x0 y0
    }
    
    HotkeyChanged(name, newhotkey){
        MsgBox % "Hotkey '" name "' changed to: " newhotkey
    }
}
...
class _CHotkeyControl {
    ...
    _BindMode(){
        ...
        Loop % this._SelectedInput.length(){
            ...
            ;out .= this._SelectedInput[i].name
        }
